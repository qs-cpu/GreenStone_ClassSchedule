              ...
              child: Column(
                ...
                children: [
                  TextFormField(
                    ...
                    validator: (val) => (val == null || val.length < 3) ? '用户名至少 3 个字符' : null,
                  ...
                  TextFormField(
                    ...
                    obscureText: true,
                    validator: (val) => (val == null || val.length < 6) ? '密码至少 6 个字符' : null,
                    ...
                    onPressed: authState.isLoading
                        ...
                        : () {
                            if (_formKey.currentState!.validate()) {
                              ...
                              ref.read(authStateProvider.notifier).register(_username, _password, _nickname).then((_) {
                                if (!ref.read(authStateProvider).hasError) {
                                  // 注册成功并自动登录成功后
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('注册并登录成功！'), backgroundColor: Colors.green),
                                  );
                                }
                              });
                            }

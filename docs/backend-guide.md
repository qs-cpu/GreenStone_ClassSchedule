...
```typescript
import { pgTable, text, timestamp, integer, varchar, uuid, jsonb } from 'drizzle-orm/pg-core'
...
export const courses = pgTable('courses', {
  ...
  locationText: varchar('location_text', { length: 100 }),
...
export const courseSessions = pgTable('course_sessions', {
  ...
  weekType: varchar('week_type', { length: 20 }), // 'all' | 'odd' | 'even'
  note: text('note'),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
})
